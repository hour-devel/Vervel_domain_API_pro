...
<div class="tab" style="background-color:{{$back_color}}">
    <div class="form" style="background-color: {{$table_color}}">
        <form action="{{route('Layout.update',['Layout'=>$layout->id])}}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="row">
                <div class="column">
                ...
                <div class="column">
                    ...
                    <div class="box">
                        <select name="status" class="form-select @error('status') is-invalid @enderror"" style="border:2px solid {{$input_border}}">
                            <option {{ $layout->status == 1 ? "selected" : "" }} value="1">Show</option>
                            <option {{ $layout->status == 0 ? "selected" : "" }} value="0">Hide</option>
                        </select>
                        @error('status')
                            <div class="invalid-feedback">
                                {{ $message }}
