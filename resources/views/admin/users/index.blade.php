    ...
    <div class="list" style="background-color: {{$list}}">
        <table style="background-color: {{$table_color}}">
            <tr>
                ...
                <th style="color:{{$th_color}}" width="100px">Role</th>
                <th style="color:{{$th_color}}" width="">Name</th>
                <th style="color:{{$th_color}}" width="">Email</th>
                <th style="color:{{$th_color}}" width="120px">Address</th>
                <th style="color:{{$th_color}}" width="100px">BirthDate</th>
            ...
            @foreach ($users as $user)
                <tr>
                    <td style="color:{{$td_color}}"><img src="{{asset('storage/'.$user->photo)}}" alt=""></td>
                    <td style="color:{{$td_color}}">{{$user->role_id}}</td>
                    <td style="color:{{$td_color}}">{{$user->first_name.'-'.$user->last_name}}</td>
                    <td style="color:{{$td_color}}">{{$user->email}}</td>
                    <td style="color:{{$td_color}}">{{$user->address}}</td>
                    <td style="color:{{$td_color}}">{{$user->birthdate}}</td>
                    <td style="color:{{$td_color}}">{{$user->phone_num}}</td>
                    <td style="color:{{$td_color}}">{{$user->created_at}}</td>
                    <td style="color:{{$td_color}}">
                        <a href="{{route('User.show',['User'=>$user->id,'mode'=>$mode])}}" style="color:{{$td_color}}">
                ...
                @if(request()->get('limite'))
                    <div class="select">
                        <select name="" id="" class="form-select" onchange="window.location.href=this.options[this.selectedIndex].value;">
                            ...
                            <option {{ request()->get('limite') == 10 ? "selected" : "" }} value="{{route('User.index',['mode'=>$mode,'limite'=>10])}}">10</option>
                            <option {{ request()->get('limite') == 20 ? "selected" : "" }} value="{{route('User.index',['mode'=>$mode,'limite'=>20])}}">20</option>
                            <option {{ request()->get('limite') == 100 ? "selected" : "" }} value="{{route('User.index',['mode'=>$mode,'limite'=>100])}}">100</option>
                        </select>
                    </div>
                @else
                    <div class="select">
                ...
                <div class="action">
                    ...
                    <div class="next">
                        @if ($users->hasMorePages())
                            ...
                            <a href="{{ \url()->full().'&page='.$users->lastPage() }}">
